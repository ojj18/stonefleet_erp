...
class ExcavatorServiceScreen extends StatefulWidget {
  const ExcavatorServiceScreen({super.key});

  @override
  State<ExcavatorServiceScreen> createState() => _ExcavatorServiceScreenState();
}
...
class _ExcavatorServiceScreenState extends State<ExcavatorServiceScreen> {
  ...
  Widget _buildSummaryCards() {
    return Consumer<ExcavatorServiceProvider>(
    ...
    return Consumer<ExcavatorServiceProvider>(
      builder: (context, provider, child) {
        ...
        final filtered = _filteredServices(
          provider.services,
          context.read<ExcavatorProvider>().excavators,
        );
  ...
  Widget _buildServiceRow(
    ExcavatorServiceModel service,
    List<ExcavatorModel> excavators,
  ) {
    final excavator = _findExcavator(service.excavatorId, excavators);
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              flex: 2,
              child: Text(_formatNumber(service.currentHourMeter)),
            ),
            Expanded(
              flex: 1,
  ...
  List<ExcavatorServiceModel> _filteredServices(
    List<ExcavatorServiceModel> services,
    List<ExcavatorModel> excavators,
  ) {
  ...
  Future<void> _openAddScreen() async {
    final result = await Navigator.push<bool>(
      context,
      MaterialPageRoute(builder: (_) => const ExcavatorServiceAddEditScreen()),
    );
  ...
  Future<void> _openEditScreen(ExcavatorServiceModel service) async {
    ...
    final result = await Navigator.push<bool>(
      context,
      MaterialPageRoute(
        builder: (_) => ExcavatorServiceAddEditScreen(serviceId: service.id!),
      ),
    );
  ...
  Future<void> _confirmDelete(ExcavatorServiceModel service) async {
    if (service.id == null) return;

    final registration = _findExcavator(
      service.excavatorId,
      context.read<ExcavatorProvider>().excavators,
    )?.registrationNumber;
  ...
  Future<_ItemSummary> _getItemSummary(int? serviceId) async {
    ...
    final repository = ExcavatorServiceRepository();
  ...
  ExcavatorModel? _findExcavator(
    int excavatorId,
    List<ExcavatorModel> excavators,
  ) {
    for (final excavator in excavators) {
      if (excavator.id == excavatorId) {
        return excavator;
      }
    }
